...
def test_from_sitk(image_mha):
    sitk_img = sitk.ReadImage(image_mha)

    img = from_sitk(image_mha)
    img.compute()
    ...
    img = from_sitk(image_mha, chunks=(1, -1, -1))
    ...
    img = from_sitk(image_mha, chunks=(1, 128, 128))
    img.compute()
    assert [c[0] for c in img.chunks] == [1, 128, 128]
    assert np.array_equal(np.asarray(img), sitk.GetArrayViewFromImage(sitk_img))
    assert img.dtype == sitk.extra._get_numpy_dtype(sitk_img)

    img = from_sitk(image_mha, chunks=(-1, 83, 89))
    img.compute()
...
def test_from_sitk_2d(image_mha_2d):
    sitk_img = sitk.ReadImage(image_mha_2d)

    img = from_sitk(image_mha_2d)
    img.compute()
    assert img.shape == sitk_img.GetSize()[::-1]
    assert np.array_equal(np.asarray(img), sitk.GetArrayViewFromImage(sitk_img))
    assert img.dtype == sitk.extra._get_numpy_dtype(sitk_img)

    img = from_sitk(image_mha_2d, chunks=(128, -1))
    img.compute()
    assert [c[0] for c in img.chunks] == [128, sitk_img.GetSize()[0]]
    assert np.array_equal(np.asarray(img), sitk.GetArrayViewFromImage(sitk_img))
    assert img.dtype == sitk.extra._get_numpy_dtype(sitk_img)

    img = from_sitk(image_mha_2d, chunks=(128, 128))
    img.compute()
